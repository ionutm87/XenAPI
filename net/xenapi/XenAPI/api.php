    ...
    */
    public function processRequest() {
        ...
        case 'createprofilepost':
            ...
            case 'drupal-login':
                /**
                 * Logins the user form drupal.
                 *
                 * EXAMPLE:
                 *   - api.php?action=login&username=USERNAME&password=PASSWORD&ip_address=xf.forum.dev
                 */
                if (!$this->hasRequest('username')) {
                    // The 'username' argument has not been set, throw error.
                    $this->throwError(3, 'username');
                ...
                if (!$user->isRegistered()) {
                    ...
                    // Requested user was registered, check authentication.
                    if ($user->validateDrupalAuthentication($user, $this->getRequest('password'))) {
                        // Authentication was valid, grab the user's authentication record.
                        $record = $user->getAuthenticationRecord();
                        $ddata = unserialize($record['data']);
...
class User {
    ...
    */
    ...
    public function validateDrupalAuthentication($user, $password) {
        if ($password === md5(APIKEY . $user->data['username'])) {
            return true;
        }
    ...
    protected function _getPassword($user_name)
    {
        $secret = XenForo_Application::get('config')->drupal_secret;
        return md5("$secret $user_name");
    }
